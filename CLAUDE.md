...
- docs/agents-foundation-v1.md — V1 spec unifying Skills (predicate-triggered context loading reusing engine ext 2 grammar) + Sub-agents (deterministic state-machine, async background, phase-bounded budgets, hard no-write floor) under a new Agents sidebar tab. **SHIPPED 2026-05-06: M1+M2+M3+M4+M5 all live** — schema (skills/skill_invocations/sub_agent_definitions/sub_agent_sessions/sub_agent_steps/sub_agent_findings + agent_actions.sub_agent_session_id), db helpers (24 new), modules (chatContext, skillLoader, subAgents/{policy, progress, orchestrator, researchAgent, worker, notify}), Aria chat tools (10 new: 6 skill CRUD + 4 sub-agent control), REST routes (/api/skills/* + /api/sub-agents/*), AgentsPanel.jsx UI (landing + skill edit + run detail + dispatch modal), CC indicators (📚 loaded skill chip + 🔬 sub-agent dispatch tile), system prompt directives, /api/admin/agents-health endpoint, async worker booted from proxy-server.cjs. 132 tests passing. (Claude + Lyle, May 6)
