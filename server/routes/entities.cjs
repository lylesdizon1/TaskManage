...
module.exports = function createEntitiesRouter({ authenticateToken, requireAdmin, db }) {
  ...
  router.post('/api/entities', authenticateToken, requireAdmin, async (req, res) => {
    try {
  ...
  router.put('/api/entities/:id', authenticateToken, requireAdmin, async (req, res) => {
    try {
  ...
  router.delete('/api/entities/:id', authenticateToken, requireAdmin, async (req, res) => {
    try {
      const existing = await db.getEntityById(req.params.id);
      if (!existing) return res.status(404).json({ error: 'Entity not found' });
      if (existing.shared && existing.createdBy !== req.user.id && req.user.role !== 'admin') {
