...
| **Concurrent sub-agent runs from one user** | V1 cap: 3 concurrent runs per user, server-enforced. Excess `start_sub_agent` calls return `queued` status until a slot opens. Watching the queued state is part of the Agents tab UI. |
